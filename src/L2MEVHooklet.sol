...
contract L2MEVHooklet is IHooklet {
        ...
        returns (bool feeOverriden, uint24 fee, bool priceOverridden, uint160 sqrtPriceX96)
    {
        ...
        if (address(config.oracle) != address(0) && isTopOfBlockSwap) {
            ...
            bool token0IsSpecified = exactIn == params.zeroForOne;
            (address base, address quote) = token0IsSpecified
            ...
            (uint256 amount0, uint256 amount1) = token0IsSpecified ? (inAmount, outAmount) : (outAmount, inAmount);
            if (amount0 == 0) {
        ...
        if (config.priorityFeeMultiplier != 0) {
            ...
            if (amountSpecifiedIsInETH) {
                ...
                // use sqrtPriceX96 to compute the corresponding ETH value
                // and then compute the fee rate
                (uint160 sqrtPriceX96_,,,) = IBunniHook(address(key.hooks)).slot0s(poolId);
                uint256 amountSpecified = params.amountSpecified.abs();
                if (currency0IsEth) {
                    ...
                    swapAmountInWei = amountSpecified.fullMulDiv(priceOf1In0X64, Q64);
                } else {
                    ...
                    uint256 priceOf0In1X64 = sqrtPriceX64.mulDiv(sqrtPriceX64, Q64); // unit: token0 / token1 in Q64
                    swapAmountInWei = amountSpecified.fullMulDiv(priceOf0In1X64, Q64);
                }
            }
